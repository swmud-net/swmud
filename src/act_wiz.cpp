...
DEF_DO_FUN( wizinfo )
{
	...
	{
		send_to_char("Nie masz wystarczaj�cych uprawnie�." NL, ch);
		return;
	}
...
DEF_DO_FUN( authorize )
{
	...
	if (arg2[0] == '\0' || !str_cmp(arg2, "accept") || !str_cmp(arg2, "yes"))
	{
		...
		ch_printf(victim,
				"" NL /* B */
				"                                  &W%s!&w                                " NL /* B */
				"                  &WAdministratorzy zaakceptowali twoje imi�.&w          " NL /* B */
	...
	{
		send_to_char("Tw�j dost�p do muda zosta� zabroniony." NL, victim);
	...
	else if (!str_cmp(arg2, "name") || !str_cmp(arg2, "n"))
	{
		...
		ch_printf(victim,
				...
				"Wymy�l inne, bardziej odpowiednie i poinformuj o tym Administrator�w." NL,
				victim->name);
...
DEF_DO_FUN( deny )
{
	...
	send_to_char("Masz zabroniony dost�p do muda!" NL, victim);
	send_to_char("OK." NL, ch);
	do_quit(victim, (char*) "");
...
DEF_DO_FUN( fquit )
{
	...
	send_to_char("Administratorzy muda ka�� Ci wyj�� z gry." NL, d->character);
...
DEF_DO_FUN( transfer )
{
	...
	act( COL_FORCE, "$n znika rozpadaj�c si� na male�kie cz�steczki.", victim,
			NULL, NULL, TO_ROOM);
	victim->retran = victim->in_room->vnum;
	char_from_room(victim);
	char_to_room(victim, location);
	act( COL_FORCE, "$n materializuje si� tutaj.", victim, NULL, NULL, TO_ROOM);
	if (ch != victim)
		act( COL_IMMORT, "$n przenosi ci�.", ch, NULL, victim, TO_VICT);
...
DEF_DO_FUN( at )
{
	...
	for (auto* wch : char_list)
	{
		if (wch == ch)
		{
			...
			/* BUGFIX by Thanos:
			 * po wykonaniu komendy progowej mob cz�sto u�ywa mpat, a co
			 * za tym idzie robi co� w innej lokacji, czyli wypina si� z listy
			 * lokacji aktualnej. Po powrocie zostaje wpi�ty na sam jej koniec,
			 * a co za tym idzie, p�tla lec�ca po wszystkich w pomieszczeniu
			 * zn�w trafia na niego. Taki manewr grozi zap�tlaniem i duplikowaniem
			 * si� wykona� prog�w. M�j spos�b: wepnijmy mobka tam gdzie by�.
...
DEF_DO_FUN( rat )
{
	...
	if (IS_NPC(ch)) /*bez zb�dnego ryzyka*/
	{
		huh(ch);
		return;
	}
	...
	/* je�li kto� zainteresowany chce sprawdzi� co tak zapierdala :) */
...
DEF_DO_FUN( rstat )
{
	...
	if (get_trust(ch) < LEVEL_IMMORTAL)
	{
		...
		{
			send_to_char(
					"Mo�esz u�ywa� rstat tylko w przydzielonej ci krainie." NL,
					ch);
			return;
		}
	}
	...
	if (ch->in_room != location && room_is_private(ch, location))
	{
		...
		{
			send_to_char("Omijam flag� 'private'!" NL, ch);
		}
...
DEF_DO_FUN( mstat )
{
			...
			|| ( IS_ADMIN( victim->name ) && !IS_ADMIN(ch->name))) //added by Thanos :)
	{
		...
		"%s blask o�lepia ci� nie pozwalaj�c przyjrze� si� bli�ej." EOL,
...
DEF_DO_FUN( reboot )
{
	...
	NL NL "dostrzec"
	NL NL "jak odradza si� �ycie �wiata..." NL, ECHOTAR_ALL);
...
DEF_DO_FUN( return )
{
	...
	send_to_char("Tw�j umys� powraca do swojego cia�a." NL, ch);
...
DEF_DO_FUN( purge )
{
	...
	{
// removed by Thanos. Ze wzgl�du na bezpiecze�stwo (OLC)
// purge robimy tylko w lokacji, w kt�rej jeste�my.
//	if ( ( victim = get_char_world( ch, arg ) ) == NULL
...
//blee    act( COL_IMMORT, "$n dotyka d�oni� $N$3. $N rozpada si� na ma�e kawa�eczki!", ch, NULL, victim, TO_NOTVICT );
...
DEF_DO_FUN( balzhur )
{
	...
	send_to_char("Budzisz si� po d�ugiej chwili..." NL, victim);
...
DEF_DO_FUN( advance )
{
	...
	{
		send_to_char("Poziom musi by� z przedzia�u 1 do 500." NL, ch);
		return;
	}
	...
	if (level <= victim->skill_level[ability])
	{
		...
		send_to_char( COL_IMMORT, victim);
		ch_printf(victim, "Przekl�t%s! W�adcy obni�aj� tw�j poziom!" EOL,
	...
	else
	{
		...
		send_to_char("W�adcy nagradzaj� ci� poziomem!" NL, victim);
	}
...
DEF_DO_FUN( immadvance )
{
	...
	act( COL_IMMORT,
			"$n przygl�da ci si� z uwag�... potem wznosi swe r�ce do g�ry...",
			ch, NULL, NULL, TO_ROOM);
	send_to_char( FB_WHITE, victim);
	send_to_char("Wszystko staje si� takie dziwne..." NL NL, victim);
	send_to_char( FB_CYAN, victim);

	sprintf(buf, "M_GODLVL%d_", victim->top_level - 99);
	do_help(victim, buf);
	send_to_char( FB_WHITE, victim);
	send_to_char("Budzisz si�... a wszystkie niedoskona�o�ci znikaj�." EOL,
			victim);
	while (!victim->carrying.empty())
		extract_obj(victim->carrying.front());
...
/* Moim zdaniem restore powinien od�wie�a� posta� a nie tylko hp, force i mv
...
DEF_DO_FUN( restore )
{
	...
	if (!str_cmp(arg, "all"))
	{
		...
		{ auto snapshot = char_list; for (auto* vch : snapshot)
		{
			...
			{
				restore_char(vch);
				ch_printf(vch, "%s przywraca ci pe�ni� si�." EOL,
						PERS(ch, vch, 0));
			}
		} }
	}
	else
	{
		...
		restore_char(victim);
		if (ch != victim)
			ch_printf(victim, "%s przywraca ci pe�ni� si�." EOL,
					PERS(ch, victim, 0));
		send_to_char("Ok." NL, ch);
		return;
	}
...
DEF_DO_FUN( freeze )
{
	...
	if (IS_SET(victim->act, PLR_FREEZE))
	{
		...
		send_to_char("Mo�esz zn�w gra�." NL, victim);
		send_to_char("FREEZE usuni�ty." NL, ch);
	}
	else
	{
		SET_BIT(victim->act, PLR_FREEZE);
		send_to_char("Nie mo�esz zrobi� NICZEGO!" NL, victim);
...
DEF_DO_FUN( litterbug )
{
	...
	if (IS_SET(victim->act, PLR_LITTERBUG))
	{
		...
		send_to_char("Mo�esz zn�w odrzuca� �mieci." NL, victim);
		send_to_char("LITTERBUG removed." NL, ch);
	}
	else
	{
		SET_BIT(victim->act, PLR_LITTERBUG);
		send_to_char("Co� nie pozwala ci odrzuca� przedmiot�w!" NL, victim);
...
DEF_DO_FUN( rename )
{
	 ...
	 * razu. Tworzymy sobie posta� tymczasow� i zapami�tujemy na ni� wska�nik.
	 * Immo mo�e teraz bezstresowo edytowa� posta� tymczasow�, a jak sko�czy,
	 * jej dane zostan� skopiowane na gracza.
	 * Dodatkowo, �eby by�o wygodnie, nasz edytor b�dzie pyta� o przypadki po
	 * kolei. Numer aktualnie edytowanego przypadku mamy zapami�tany w ...
...
DEF_DO_FUN( force )
{
	...
	if (!str_cmp(arg, "all"))
	{
		...
		{ auto snapshot = char_list; for (auto* vch : snapshot)
		{
			...
			{
				act( COL_IMMORT,
						"Za spraw� $n$1 czujesz, �e dobrze by�oby zrobi�: '$t'.",
	...
	else
	{
		...
		act( COL_IMMORT,
				"Za spraw� $n$1 czujesz, �e dobrze by�oby zrobi�: '$t'.", ch,
...
DEF_DO_FUN( invis )
{
	...
	if (*arg)
	{
		...
		else if (!str_cmp(arg, "on"))
		{
			...
			act( COL_IMMORT, "$n powoli zlewa si� z otoczeniem.", ch, NULL,
					NULL, TO_ROOM);
		...
		else if (!str_cmp(arg, "off"))
		{
			...
			act( COL_IMMORT, "$n powoli materializuje si�.", ch, NULL, NULL,
			TO_ROOM);
	...
	{
		SET_BIT(ch->act, PLR_WIZINVIS);
		act( COL_IMMORT, "$n powoli zlewa si� z otoczeniem.", ch, NULL, NULL,
		TO_ROOM);
		send_to_char("Wizinvis set." NL, ch);
	}
...
DEF_DO_FUN( loadup )
{
	...
	if (stat(fname, &fst) != -1)
	{
		...
		{
			do_say(d->character, (char*) "*NIE* przeszkadza�!");
			send_to_char("T� osob� lepiej zostawi� w spokoju!" NL, ch);
...
DEF_DO_FUN( newbieset )
{
	...
	act( COL_IMMORT, "$n wyposa�y�$o ci� w zestaw m�odego gracza.", ch, NULL,
			victim, TO_VICT);
...
void do_set_password(CHAR_DATA *ch, char *argument) //Trog
{
	...
	{
		ch_printf(ch, "Has�o musi mie� co najmniej %d znak�w." NL,
		MIN_PASS_LEN);
		return;
	}
...
/*changed by Thanos:
 zmieni�em t� funkcj� tak, by dzia�a�a fragmentami, co pulse_for (ok. 1s),a
 nie naraz tak jak by�o dotychczas. Teraz funkcja pulsuje sobie w tle nie
 laguj�c a gracz kt�ry wykona� t� komend� ma wolny prompt.
...
//Trog: od tad do komentarza powyzej jest to syfiaste i zle dziala, poprawie niedlugo.
	...
	/* czy�cimy star� p�tl� (je�li jest) tylko w przypadku, gdy mamy ju�
	 wszystkie potrzebne nam warto�ci */
	clean_for_loop();
	/* kopiujemy warto�ci */
	/*ustalamy zakres p�tli*/
...
DEF_DO_FUN( cset )
{
	...
	/*Added by Thanos: czy mud ma logowa� np. resety krain */
...
DEF_DO_FUN( isolate )
{
	...
	do_look(victim, (char*) "auto");
	ch_printf(victim,
			"Mistrzowie Jedi nie s� zadowoleni z twojego post�powania." NL
			"Zostaniesz w izolatce przez %d %s." NL, time,
			h_d ? (time == 1 ? "godzin�" : "godzin") : (
					time == 1 ? "dzie�" : "dni"));
...
DEF_DO_FUN( unsilence )
{
	...
	send_to_char("Mistrzowie Jedi przebaczaj� Ci! Pozwalaj� Ci przem�wi�." NL,
			victim);
...
/*
 * Thanos - r�wnie� ze Smauga
 * funkcja pokazuje zawarto�ci r�norakich plik�w
 *
