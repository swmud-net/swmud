...
int get_obj_resistance(OBJ_DATA *obj)
{
	...
	/* Thanos -->   �enada, �e komlinki rozpadaj� si� jako pierwsze */
...
void show_obj_stats(OBJ_DATA *obj, CHAR_DATA *ch)
{
	...
	case ITEM_WEAPON:
		ch_printf(ch, FG_CYAN
		"Zadawane obra�enia w walce wr�cz " FB_CYAN "%d" FG_CYAN
		...
		if (obj->value[3] == WEAPON_BLASTER)
		{
			if (obj->blaster_setting == BLASTER_FULL)
				...
				ch_printf(ch, FG_CYAN "Ustawiony na PO�OW� mocy." EOL);
			...
			ch_printf(ch, FG_CYAN
			"Posiada jeszcze " FB_CYAN "%d" FG_CYAN " na " FB_CYAN "%d" FG_CYAN " pocisk�w." EOL, obj->value[4],
					obj->value[5]);
		}
	...
	case ITEM_DEVICE:
		...
		") �adunk�w na poziomie " FB_CYAN "%d ", obj->value[1], obj->value[2], obj->value[0]);
	...
	case ITEM_AMMO:
		ch_printf(ch, FG_CYAN
		"Posiada " PLAIN "%d" FG_CYAN " pocisk�w." EOL, obj->value[0]);
		break;
...
void get_obj(CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container)
{
	...
	{
		send_to_char("Nie mo�esz tego wzi��." NL, ch);
		return;
	}

	if (IS_OBJ_STAT( obj, ITEM_PROTOTYPE ) && !can_take_proto(ch))
	{
		send_to_char("Nieznana moc powstrzymuje ci� przed podej�ciem by to wzi��." NL, ch);
		return;
	}
	...
	{
		ch_printf(ch, "To chyba nie twoje. Spad�wa!" NL "Nie mo�esz tego wzi��." NL);
		return;
	}
	...
	if (obj->item_type != ITEM_MONEY)
	{
		...
		{
			act(PLAIN, "$d: Nie mo�esz nosi� tylu przedmiot�w.", ch, NULL, obj->name, TO_CHAR);
			return;
		}
		...
		{
			act(PLAIN, "$d: Nie uniesiesz takiego ci�aru.", ch, NULL, obj->name, TO_CHAR);
			return;
		}
	}
...
DEF_DO_FUN( get )
{
	...
	if (is_number(arg1))
	{
		...
		if (number < 1)
		{
			send_to_char("To by�o naprawd� �atwe..." NL, ch);
			return;
		}
	...
	if (arg2[0] == '\0')
	{
		...
		else
		{
			...
			{
				send_to_char("W�adcy �miej� si� z twojej chciwo�ci!" NL, ch);
				return;
			}
			if (!str_cmp(arg1, "all"))
				fAll = true;
			...
			found = false;
			{ auto snapshot = ch->in_room->contents; for (auto* obj : snapshot)
			{
	...
	else
	{
		...
		default:
			...
			{
				send_to_char("Zbyt ci�ko by ud�wign��." NL, ch);
				return;
			}
			break;
		...
		{
			act(PLAIN, "$d jest zamkni�ty.", ch, NULL, container->name, TO_CHAR);
			return;
		}
		...
		else
		{
			...
			{
				{ auto snapshot = container->contents; for (auto* obj : snapshot)
				{
					if ((fAll || nifty_is_name(chk, obj->name)) && can_see_obj(ch, obj))
					{
						found = true;
...
DEF_DO_FUN( put )
{
	...
	{
		send_to_char("W�o�y� co do czego?" NL, ch);
		return;
	}
	...
	if (number <= 1 && str_cmp(arg1, "all") && str_prefix("all.", arg1))
	{
		...
		if (obj == container)
		{
			send_to_char("Hmm.. Nie da si� w�o�y� przedmiotu do niego samego." NL, ch);
			return;
		}
		 ...
		 *{
		 *    ch_printf( ch,
		 *    "Przecie� ty i %s" PLAIN " to tak zgrany duet. Nie rozstaniecie si� ani na chwil�!" NL,
		 *	obj->przypadki[0] );
		 *    return;
		 *}
		...
		{
			send_to_char("To si� nie zmie�ci." NL, ch);
			return;
		}
	...
	else
	{
		...
		{ auto snapshot = ch->carrying; for (auto* obj : snapshot)
		{
...
DEF_DO_FUN( drop )
{
	...
	if (IS_SET( ch->in_room->room_flags, ROOM_NODROP ) || (!IS_NPC(ch) && IS_SET( ch->act, PLR_LITTERBUG )))
	{
		...
		send_to_char("Moc powstrzymuje ci�!" EOL, ch);
		send_to_char(COL_TELL, ch);
		send_to_char("Kto� m�wi ci 'Bez �miecenia prosz�!'" EOL, ch);
		return;
	}
	...
	if (number <= 1 && str_cmp(arg, "all") && str_prefix("all.", arg))
	{
		...
		if (!can_drop_obj(ch, obj))
		{
			send_to_char("Nie mo�esz go odrzuci�." NL, ch);
			return;
		}
...
DEF_DO_FUN( give )
{
	...
	if (is_number(arg1))
	{
		...
		{
			send_to_char("Przykro mi, nie mo�esz tego zrobi�." NL, ch);
			return;
		}
		...
		{
			send_to_char("P�ki co sam potrzebujesz tej got�wki. To nie sk�pstwo, to zdrowy rozs�dek!" NL, ch);
			return;
		}
		...
		strcat(buf, arg1);
		strcat(buf, (amount > 1) ? (amount < 5) ? " kredytki." : " kredytek." : " kredytk�.");
	...
	{
		send_to_char("Musisz to najpierw zdj��." NL, ch);
		return;
	}
	...
	/* Thanos: 	questy; (can_drop_obj nie pozwala pozby� si� przedmiotu)
	 Tutaj mo�emy to zignorowa� bo odbiorca �eby wzi�� przedmiot
	 questowy MUSI by� w que�cie */
	...
	{
		act(PLAIN, "$N ma ju� pe�ne r�ce.", ch, NULL, victim, TO_CHAR);
		return;
	}
	...
	{
		act(PLAIN, "$N nie uniesie takiego ci�aru.", ch, NULL, victim, TO_CHAR);
		return;
	}
	...
	{
		act(PLAIN, "Nie mo�esz da� $p$1 $N$2!", ch, obj, victim, TO_CHAR);
		return;
	}
...
obj_ret damage_obj(OBJ_DATA *obj)
{
	...
	if (ch)
		act(COL_OBJECT, obj->gender == GENDER_NEUTRAL ? "$p zostaje uszkodzone"
				...
				NULL, TO_CHAR);
	else if (obj->in_room && !obj->in_room->people.empty() && (ch = obj->in_room->people.front()) != NULL)
	{
		act(COL_OBJECT, obj->gender == GENDER_NEUTRAL ? "$p zostaje uszkodzone"
				: obj->gender == GENDER_MALE ? "$p zostaje uszkodzony"
						: obj->gender == GENDER_FEMALE ? "$p zostaje uszkodzona" : "$p zostaj� uszkodzone", ch, obj,
...
bool remove_obj(CHAR_DATA *ch, int iWear, bool fReplace)
{
	...
	{
		act(PLAIN, "$d: Nie mo�esz nie�� tylu przedmiot�w.", ch, NULL, obj->name, TO_CHAR);
		return false;
	}
	...
	{
		act(PLAIN, "Nie mo�esz zdj�� $p$1.", ch, obj, NULL, TO_CHAR);
		return false;
	}
...
bool can_dual(CHAR_DATA *ch)
{
	...
	{
		send_to_char("Nie mo�esz dzier�y� dw�ch broni nosz�c co� w r�ku!" NL, ch);
		return false;
	}

	if ((tmpobj = get_eq_char(ch, WEAR_WIELD)) != NULL && IS_SET( tmpobj->wear_flags, ITEM_DUAL_WIELD ))
	{
		send_to_char("Masz zaj�te obie r�ce!" NL, ch);
		return false;
	}
	return true;
}
...
void wear_obj(CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace, int wear_bit)
{
	...
	if (wear_bit > -1)
	{
		...
		if (!CAN_WEAR(obj, 1 << bit))
		{
			if (fReplace)
			{
				...
				{
				case ITEM_HOLD:
					send_to_char("Nie mo�esz tego trzyma�." NL, ch);
					break;
				case ITEM_WIELD:
				case ITEM_DUAL_WIELD:
					send_to_char("Nie mo�esz tego dzier�y�." NL, ch);
					break;
				default:
					sprintf(buf, "Nie mo�esz tego za�o�y�.\n\r");
					send_to_char(buf, ch);
				}
			}
			return;
	...
	if (check_size)
	{
		...
		{
			act(COL_FORCE,
					"Prawdopodobnie niemo�liwo�ci� b�dzie znale�� co� co pasowa�oby na twoje\n\rnienaturalne cia�o.",
					ch, NULL, NULL, TO_CHAR);
			act(PLAIN, "$n pr�buje u�y� $p$1, ale jest zbyt du�$y.", ch, obj, NULL, TO_ROOM);
			return;
		}
		...
		if (IS_OBJ_STAT(obj, ITEM_HUTT_SIZE))
		{
			...
			act(PLAIN, "$n pr�buje u�y� $p$1, ale jest za ma�$y.", ch, obj, NULL, TO_ROOM);
			return;
		}

		if (IS_OBJ_STAT(obj, ITEM_LARGE_SIZE) || IS_OBJ_STAT(obj, ITEM_HUMAN_SIZE))
		{
			act(COL_FORCE, "To nie tw�j rozmiar.", ch, NULL, NULL, TO_CHAR);
			act(PLAIN, "$n pr�buje u�y� $p$1, ale jako� nie mo�e.", ch, obj, NULL, TO_ROOM);
			return;
		}
	...
	{
		if (fReplace)
			send_to_char("Nie mo�esz tego za�o�y�, dzier�y� ani trzyma�." NL, ch);
		return;
	}
	...
	case ITEM_WEAR_FINGER:
		...
		if (!get_eq_char(ch, WEAR_FINGER_L))
		{
			...
			{
				if (!obj->action_desc || obj->action_desc[0] == '\0')
					act(PLAIN, "Wsuwasz $p$3 na palec lewej r�ki.", ch, obj, NULL, TO_CHAR);
				else
					actiondesc(ch, obj, NULL);

				act(PLAIN, "$n wsuwa $p$3 na palec lewej r�ki.", ch, obj, NULL, TO_ROOM);
			}
		...
		if (!get_eq_char(ch, WEAR_FINGER_R))
		{
			...
			{
				if (!obj->action_desc || obj->action_desc[0] == '\0')
					act(PLAIN, "Wsuwasz $p$3 na palec prawej r�ki.", ch, obj, NULL, TO_CHAR);
				else
					actiondesc(ch, obj, NULL);

				act(PLAIN, "$n wsuwa $p$3 na palec prawej r�ki.", ch, obj, NULL, TO_ROOM);
			}
	...
	case ITEM_WEAR_BODY:
		...
		{
			send_to_char("Nie mo�esz za�o�y� tego na to, co ju� nosisz." NL, ch);
			return;
		}
		if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL))
		{
	...
	case ITEM_WEAR_HEAD:
		...
		{
			send_to_char("Nie mo�esz nosi� nic na g�owie." NL, ch);
			return;
		}
		...
		{
			if (!obj->action_desc || obj->action_desc[0] == '\0')
				act(PLAIN, "Nak�adasz sobie $p$3 na g�ow�.", ch, obj, NULL, TO_CHAR);
			else
				actiondesc(ch, obj, NULL);

			act(PLAIN, "$n nak�ada sobie $p$3 na g�ow�.", ch, obj, NULL, TO_ROOM);
		}
	...
	case ITEM_WEAR_EARS:
		...
		{
			send_to_char("Nie mo�esz za�o�y� tego na uszy bo... nie masz uszu. :(" NL, ch);
			return;
		}
	...
	case ITEM_WEAR_LEGS:
		...
		if (IS_RACE( ch, "HUTT" ))
		{
			send_to_char("Wiesz... Huttowie nie maj� n�g, wi�c..." NL, ch);
			return;
		}
	...
	case ITEM_WEAR_FEET:
		...
		if (IS_RACE( ch, "HUTT" ))
		{
			send_to_char("Wiesz... Huttowie nie maj� st�p..." NL, ch);
			return;
		}
	...
	case ITEM_WEAR_HANDS:
		...
		{
			send_to_char("Nie mo�esz za�o�y� tego na co� co ju� nosisz." NL, ch);
			return;
		}
		if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL))
		{
			if (!obj->action_desc || obj->action_desc[0] == '\0')
				act(PLAIN, "Zak�adasz $p$3 na r�ce.", ch, obj, NULL, TO_CHAR);
			else
				actiondesc(ch, obj, NULL);

			act(PLAIN, "$n zak�ada $p$3 na r�ce.", ch, obj, NULL, TO_ROOM);
		}
	...
	case ITEM_WEAR_SHIELD:
		...
		{
			if (!obj->action_desc || obj->action_desc[0] == '\0')
				act(PLAIN, "Zak�adasz $p$3 jako tarcz� energetyczn�.", ch, obj, NULL, TO_CHAR);
			else
				actiondesc(ch, obj, NULL);

			act(PLAIN, "$n zak�ada $p$3 jako tarcz� energetyczn�.", ch, obj, NULL, TO_ROOM);
		}
	...
	case ITEM_WIELD:
		...
		{
			send_to_char("Ju� dzier�ysz bro�." NL, ch);
			return;
		}
		...
		if (tmpobj)
		{
			if (can_dual(ch))
			{
				...
				{
					if (!obj->action_desc || obj->action_desc[0] == '\0')
						act(PLAIN, "Zak�adasz $p$3 jako drug� bro�.", ch, obj, NULL, TO_CHAR);
					else
						actiondesc(ch, obj, NULL);

					act(PLAIN, "$n zak�ada $p$3 jako drug� bro�.", ch, obj, NULL, TO_ROOM);
				}
		...
		{
			if (!obj->action_desc || obj->action_desc[0] == '\0')
				act(PLAIN, "Zaczynasz u�ywa� $p$1 jako broni.", ch, obj, NULL, TO_CHAR);
			else
				actiondesc(ch, obj, NULL);

			act(PLAIN, "$n zaczyna u�ywa� $p$1 jako broni.", ch, obj, NULL, TO_ROOM);
		}
	...
	case ITEM_DUAL_WIELD:
		if (!can_twohanded(ch))
		{
			send_to_char("Nie umiesz u�ywa� broni dwur�cznych." NL, ch);
			return;
		}
		...
		if (get_eq_char(ch, WEAR_HOLD))
		{
			send_to_char("Nie mo�esz dzier�y� broni dwur�cznej nosz�c co� w r�ku!" NL, ch);
			return;
		}
		...
		{
			if (!obj->action_desc || obj->action_desc[0] == '\0')
				act(PLAIN, "Zak�adasz $p$3 jako bro� dwur�czn�.", ch, obj, NULL, TO_CHAR);
			else
				actiondesc(ch, obj, NULL);
	...
	case ITEM_HOLD:
		if (get_eq_char(ch, WEAR_DUAL_WIELD))
		{
			send_to_char("Nie mo�esz nosi� czego� w r�ce kiedy dzier�ysz dwie broni!" NL, ch);
			return;
		}
		if ((tmpobj = get_eq_char(ch, WEAR_WIELD)) != NULL && IS_SET( tmpobj->wear_flags, ITEM_DUAL_WIELD))
		{
			send_to_char("Nie mo�esz nosi� czego� w r�ce kiedy dzier�ysz bro� dwur�czn�!" NL, ch);
			return;
		}
		...
		if (obj->item_type == ITEM_DEVICE || obj->item_type == ITEM_GRENADE || obj->item_type == ITEM_FOOD
				...
				NULL, NULL))
		{
			act(PLAIN, "$n bierze w r�ce $p$3.", ch, obj, NULL, TO_ROOM);
			act(PLAIN, "Bierzesz w r�ce $p$3.", ch, obj, NULL, TO_CHAR);
		}
...
DEF_DO_FUN( wear )
{
	...
	if (((!str_cmp(arg2, "on") || !str_cmp(arg2, "upon") || !str_cmp(arg2, "around") || !str_cmp(arg2, "na")
			|| !str_cmp(arg2, "w") || !str_cmp(arg2, "wok�")) && argument[0] != '\0'))
		argument = one_argument(argument, arg2);

	if (arg1[0] == '\0')
	{
		send_to_char("Za�o�y�, ubra� co?" NL, ch);
		return;
	}
...
DEF_DO_FUN( bury )
{
	...
	if (arg[0] == '\0')
	{
		send_to_char("Co sobie �yczysz zakopa�?" NL, ch);
		return;
	}
	...
	{
		act(PLAIN, "Nie mo�esz zakopa� $p$1.", ch, obj, 0, TO_CHAR);
		return;
	}
	...
	case SECT_UNDERWATER:
		send_to_char("Nie mo�esz zakopa� czego� pod wod�!" NL, ch);
		return;
	case SECT_AIR:
	...
	{
		send_to_char("�eby zakopa� co� tak wielkiego, potrzebujesz �opaty." NL, ch);
		return;
	}
	...
	if (move > ch->move)
	{
		send_to_char("Nie masz w sobie tyle energii by to zakopa�." NL, ch);
		return;
	}
	ch->move -= move;
...
void obj_fall(OBJ_DATA *obj, bool through)
{
	...
	if (IS_SET( obj->in_room->room_flags, ROOM_NOFLOOR ) && CAN_GO( obj, DIR_DOWN ) && !IS_OBJ_STAT( obj, ITEM_FORCE ))
	{
		...
		{
			act(PLAIN, "$p spada gdzie� w d�...", obj->in_room->people.front(), obj, NULL, TO_ROOM);
			act(PLAIN, "$p spada gdzie� w d�...", obj->in_room->people.front(), obj, NULL, TO_CHAR);
		}
		obj_from_room(obj);
		is_falling = true;
		...
		if (!obj->in_room->people.empty())
		{
			act(PLAIN, "$p spada sk�d� z g�ry...", obj->in_room->people.front(), obj, NULL, TO_ROOM);
			act(PLAIN, "$p spada sk�d� z g�ry...", obj->in_room->people.front(), obj, NULL, TO_CHAR);
		}
		...
		if (!IS_SET( obj->in_room->room_flags, ROOM_NOFLOOR ) && through)
		{
			...
			if (!obj->in_room->people.empty() && number_percent() > 15)
			{
				...
				for (auto* rch : obj->in_room->people) { chcnt++;
			...
			case ITEM_ARMOR:
				if ((obj->value[0] - dam) <= 0)
				{
					if (!obj->in_room->people.empty())
					{
						act(PLAIN, "$p niszczy si� przez upadek!", obj->in_room->people.front(), obj, NULL, TO_ROOM);
						act(PLAIN, "$p niszczy si� przez upadek!", obj->in_room->people.front(), obj, NULL, TO_CHAR);
					}
					make_scraps(obj);
				}
				else
